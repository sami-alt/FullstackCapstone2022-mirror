...
function Statistics() {
  ...
  const makeChartData = (all_totals) => {
    ...
    while (end.diff(start, 'days') >= 0) {
      let totals_per_day = parseFloat(
        all_totals.total_animals_daily[count].animals_today
      )
      let data1 = {
  ...
  return (
    <div>
      <h2 style={{ textAlign: 'center' }}>
        Newly admitted animals between {moment(startDate).format('MMM DD')} and{' '}
